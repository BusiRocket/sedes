...
`ventanilla-unica` authenticates to public-administration portals with the
holder's own qualified digital certificate and reads the holder's own data. The
certificate and its private key are the most sensitive material on the machine
that runs it. The tool:
...
Keep the key file mode `0600`, prefer `VENTANILLA_UNICA_CERT` and
`VENTANILLA_UNICA_KEY` over command line arguments on shared machines (arguments
are visible in the process list), and never commit certificate material. The
repository's `.gitignore` refuses `*.pem`, `*.p12`, `*.pfx`, `*.key`, `*.crt`
and `*.cer`; a `gitleaks` pre-push hook scans every push.
