import { mkdtemp, readFile, rm } from 'node:fs/promises'
import { tmpdir } from 'node:os'
import { join } from 'node:path'

import { afterEach, describe, expect, it } from 'vitest'

import { writeCensalCertificatePdf } from './writeCensalCertificatePdf'

describe('writeCensalCertificatePdf', () => {
  let dir: string | undefined

  afterEach(async () => {
    if (dir) await rm(dir, { recursive: true, force: true })
  })

  it('writes aeat-censal-<label>.pdf under outDir', async () => {
    dir = await mkdtemp(join(tmpdir(), 'censal-'))
    const path = await writeCensalCertificatePdf(
      dir,
      'B00000000-CSV1',
      Buffer.from('%PDF'),
    )
    expect(path).toBe(join(dir, 'aeat-censal-B00000000-CSV1.pdf'))
    // path is the file this same test just built under its own temp dir.
    // eslint-disable-next-line security/detect-non-literal-fs-filename
    expect((await readFile(path)).toString()).toBe('%PDF')
  })
})
